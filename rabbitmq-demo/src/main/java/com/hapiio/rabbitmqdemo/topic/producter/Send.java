package com.hapiio.rabbitmqdemo.topic.producter;
...
public class Send {
    ...
    public static void main(String[] args) throws Exception{
        ...
        for (int i = 0; i < 10; i++) {
            ...
            channel.basicPublish(EXCHANGE_NAME, "item.insert", null, insert_message.getBytes());
            System.out.println(" [商品服务] Sent '" + insert_message + "'");
            // 消息内容
            String update_message = "商品更新：，id=" + i;
            // 发送消息，并且指定routing key 为：insert ,代表新增商品
            channel.basicPublish(EXCHANGE_NAME, "item.update", null, update_message.getBytes());
            System.out.println(" [商品服务] Sent '" + update_message + "'");
            // 消息内容
            String delete_message = "商品删除：，id=" + i;
            // 发送消息，并且指定routing key 为：insert ,代表新增商品
            channel.basicPublish(EXCHANGE_NAME, "item.delete", null, delete_message.getBytes());
